...
$(function() {
    ...
    $table.bootstrapTable({
    	url: '/index.php/admin/user/limitMachine',
    	locale: "zh-CN",
    	pageSize: 20,
        pageList: [20, 10],
    	pagination: true,
    	...
    	columns: [
            ...
            { field: 'UserID',title: '管理',align:'center', formatter: function(value, row, index){
                return '<button type="button" class="btn btn-secondary btn-sm" onclick="upip(\''+row.MachineSerial+'\')"><i class="fa fa-edit"></i>&nbsp; 更新</button>&nbsp;|&nbsp;<button type="button" class="btn btn-danger btn-sm" onclick="delip(\''+row.MachineSerial+'\')"><i class="fa fa-trash-o"></i>&nbsp;删除</button>';
            }},
            { field: 'MachineSerial',title: '限制机器',align:'center'},
...
function upip(ip)
{
    jQuery.post('/index.php/admin/user/queryMachine',{ip:ip}).done(function(data){
        jQuery("#lip").val(data.MachineSerial);
        jQuery("#lip2").val(data.MachineSerial);
...
function delip(ip)
{
    var msg = "确定删除？";
    if(confirm(msg) == true){
        jQuery.post('/index.php/admin/user/delIP',{ip:ip, type: 2}).done(function(data){
...
$("#kickPeople").click(function(){
    ...
    if(confirm(msg) == true)
    {
        ...
        jQuery.each(getSelectRows, function(index, value){
            arr[index] = value['MachineSerial'];
        });
        jQuery.post('/index.php/admin/user/delsIP', {arr: arr, type: 2}).done(function(data){
            alert('删除成功');
            $table.bootstrapTable('refresh',{
                     pageNumber: 1,
            });
...
$("#editip").click(function(){
    ...
    jQuery.post('/index.php/admin/user/editIP', {obj2: obj2, type: 2}).done(function(data){
        alert('修改成功');
        $table.bootstrapTable('refresh',{
            pageNumber: 1,
        });
...
$("#addip").click(function(){
    ...
    jQuery.post('/index.php/admin/user/addMachine', {obj2: obj2}).done(function(data){
        if(data.ret == 2)
        {
            alert(data.msg);
            return false;
...
$("#iptp1").click(function(){
    $table3.bootstrapTable({
        ...
        columns: [
            ...
            { field: 'MachineSerial',title: '机器码',align:'center'},
...
$("#foreverip").click(function(){
    ...
    if(confirm(msg) == true)
    {
        ...
        jQuery.post('/index.php/admin/user/foreverIP', {arr: arr, type: 2}).done(function(data){
            alert('操作成功');
